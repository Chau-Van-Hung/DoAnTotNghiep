    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(string type, RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var nameRole = role.Equals("candidate") ? "Người tìm việc" : "Người tuyển dụng";
                var user = new ApplicationUser { UserName = model.UserName, Email = model.Email, PhoneNumber = model.Phone, EmailConfirmed = false };
                ...
                if (result.Succeeded)
                {
                    ...
                    if (role.Equals("candidate"))
                    {
                        var candidateDTO = new CandidateDTO()
                        {
                            ...
                            CPStatus = (int?)BU.Common.Enum.StatusCandidate.Active,
                    ...
                    else if (role.Equals("employer"))
                    {
                        var recruitDTO = new RecruitDTO()
                        {
                            ...
                            RIStatus = (int?)BU.Common.Enum.StatusRecruit.Active,
                            RIPhone = model.Phone,
                    ...
                    return Json(new { message = "Đăng ký thành công! Vui lòng kiểm tra email để kích hoạt tài khoản!", status = "success" }, JsonRequestBehavior.AllowGet);
                }
                else
                {
