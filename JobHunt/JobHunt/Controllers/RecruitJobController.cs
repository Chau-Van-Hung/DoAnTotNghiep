...
namespace JobHunt.Controllers
{
    public class RecruitJobController : Controller
    {
        ...
        public ActionResult DetailJob(int? id)
        {
            ...
            ViewBag.ListCandidateApplied = cddPostResumeManage.GetListCandidatePostedResume(model.RecruitJobId);
        ...
        public JsonResult getCandidates(int? id)
        {
            ...
            var ListCandidateApplied = cddPostResumeManage.GetListCandidatePostedResume(model.RecruitJobId);
            var ds = (from l1 in ListCandidateApplied
                      select new
                      {
                          avatar =  l1.CPAvatar,
                          fullName = l1.CddFullName,
                          address = l1.CddAddress,
                          id = l1.CandidateId

                      }).ToList();
            return Json(new { code = 200, listCdd = ds },JsonRequestBehavior.AllowGet);
        }
        public JsonResult getCandidatesByLevel(int? id, int value)
        {
            var model = recruitJobManage.GetRecruitJobByID((int)id);
            if (model == null)
            {
            ...
            var ListCandidateApplied = cddPostResumeManage.GetListCandidatePostedResume(model.RecruitJobId);
            var ListCandidateAppliedFiltered = ListCandidateApplied.Where(s => s.CP_LevelId == value).ToList();
            var ds = (from l1 in ListCandidateAppliedFiltered
                      select new
                      {
                          avatar = l1.CPAvatar,
                          fullName = l1.CddFullName,
        ...
        public JsonResult getCandidatesByExp(int? id, int value)
        {
            var model = recruitJobManage.GetRecruitJobByID((int)id);
            if (model == null)
            {
            ...
            var ListCandidateApplied = cddPostResumeManage.GetListCandidatePostedResume(model.RecruitJobId);
            var ListCandidateAppliedFiltered = ListCandidateApplied.Where(s => s.CP_ExperienceId == value).ToList();
            var ds = (from l1 in ListCandidateAppliedFiltered
                      select new
                      {
                          avatar = l1.CPAvatar,
        ...
        public static int SendMailToRegister(string path, string name, string tenht, string link, string mail)
        {
            ...
            try
            {
                ...
                var senderEmail = new MailAddress("vinhhuyqna@gmail.com", "dmm778631");
