...
namespace JobHunt.BU.Manage
{
    public class CandidatePostResumeManage
    {
        ...
        ConvertDataCandidate convertCandidate = new ConvertDataCandidate();
        ...
        public IEnumerable<CandidateDTO> GetListCandidatePostedResume(int? idJob)
        {
            db = new JobHuntEntities();
            List<CandidateDTO> listCandidatePostResumeDTO = new List<CandidateDTO>();
            List<Candidate> GetLists = new List<Candidate>();

            GetLists = db.CandidatePostResumes.Where(x => x.CPR_RecruitJobId == idJob).Select(x=>x.Candidate).ToList();
            foreach (var mb in GetLists)
            {
                listCandidatePostResumeDTO.Add(convertCandidate.ConvertToDTO(mb));
            }
            db = null;
            return listCandidatePostResumeDTO;
        }
    }
}
